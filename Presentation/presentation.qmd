...
## Kolik váží „typický“ savec? {.course-title .bg-indigo}
...
V jedné z pěti paralelních skupin (přibližně 26 studentů) budete analyzovat data v R s podporou vyučujících.
...
Stejná tabulka obsahuje hmotnost i spánek. Spánek nám nejprve ukáže, jak data zobrazit a popsat; potom se vrátíme k otázce o hmotnosti.
...
::: {.takeaway-strip}
Graf nevzniká místo dat. Vzniká postupným zobrazením hodnot, které jsme vložili do tabulky a vektoru. Svislá poloha teček jen brání překryvu; osa y zde neznamená další měření.
:::
...
::: {.strip-amethyst}
**Graf hustoty** vyhlazuje naměřené hodnoty do souvislé křivky. Výška ukazuje hustotu, nikoli počet druhů; jednotlivé hodnoty v něm nevidíme.
:::
...
::: {.takeaway-strip}
Minimum a maximum ukazují krajní pozorované hodnoty. Když přidáme další druhy, mohou se posunout dál; silně závisejí na velikosti souboru a neříkají, kde leží většina dat.
:::
...
U jediné hodnoty by odchylka od jejího průměru byla vždy nula. U dvou hodnot po určení průměru stačí znát jednu odchylku: druhá musí být opačná. Proto při odhadu rozptylu dělíme počtem hodnot zmenšeným o jednu.
...
::: {.takeaway-strip}
Graf nezměnil data. Jen nám dovolil vidět lehké i těžké druhy současně. Plná fialová čára je medián, přerušovaná tmavá čára průměr.
:::
...
stat_boxplot_hmotnost <-
  boxplot.stats(vec_hmotnost_kg)

q1_hmotnost <-
  stat_boxplot_hmotnost$stats[[2]]

q3_hmotnost <-
  stat_boxplot_hmotnost$stats[[4]]

vous_dolni_hmotnost <-
  stat_boxplot_hmotnost$stats[[1]]

vous_horni_hmotnost <-
  stat_boxplot_hmotnost$stats[[5]]
...
data_boxplot_hmotnost <-
  tibble::tibble(
    ...
    q1 = q1_hmotnost,
    median = median_hmotnosti,
    q3 = q3_hmotnost,
    vous_horni = vous_horni_hmotnost
  )

data_odlehle_hmotnost <-
  tibble::tibble(hmotnost_kg = stat_boxplot_hmotnost$out)
...
Vous končí u nejkrajnější pozorované hodnoty nejvýše 1,5 × IQR od okraje krabice. Body dál od krabice vyznačíme zvlášť; nejsou automaticky chybou. Pravidlo počítáme v původních kilogramech.
...
plot_hmotnost_boxplot <-
  ...
  ggplot2::geom_point(
    ...
    mapping = ggplot2::aes(x = hmotnost_kg, y = 1),
    inherit.aes = FALSE,
    color = biostat_cols[["grey_olive"]],
    size = 3
  ) +
...
[přerušovaná tmavá čára = průměr]{.text-hgl-indigo_velvet}<br>
[plná čára v krabici = medián]{.text-hgl-amethyst}
:::
:::
...
[počítáním]{.text-hgl-orange}; zde měníme jednotku pozorování na taxonomický řád. Počet mláďat má obvykle málo možných hodnot, počet listů na stromě může mít mnoho. Podle rozsahu a biologické otázky zvolíme vhodný graf i souhrn.
:::
